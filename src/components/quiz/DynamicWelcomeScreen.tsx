...
export function DynamicWelcomeScreen() {
  const { setCurrentStep, quizData } = useDynamicQuiz();
  ...
  return (
    <main className="animate-fade-in text-center max-w-2xl mx-auto" role="main" aria-labelledby="welcome-heading">
      ...
      <div className="badge-pill inline-flex items-center gap-2 mb-8" role="status">
        ...
        <span>{getText(quizData?.badge_text, 'Assessment')}</span>
      </div>
      
      <h1 id="welcome-heading" className="font-heading text-4xl md:text-5xl font-medium mb-6 leading-tight tracking-tight">
        {getText(quizData?.headline, 'Welcome')}{' '}
        <span className="font-heading italic gradient-text">{getText(quizData?.headline_highlight)}</span>
      </h1>
      
      <p className="text-lg md:text-xl text-muted-foreground mb-10 leading-relaxed max-w-xl mx-auto">
        {getText(quizData?.description)}
      </p>
      
      {getDiscoverItems().length > 0 && (
        <section className="glass rounded-xl p-6 mb-10 text-left" aria-labelledby="discover-heading">
          <h2 id="discover-heading" className="font-heading text-lg font-semibold mb-4 text-foreground">What you'll discover:</h2>
          <ul className="space-y-3" role="list">
            {getDiscoverItems().map((item, i) => (
      ...
      <p id="quiz-duration" className="text-sm text-muted-foreground mt-8">
        {getText(quizData?.duration_text, 'Takes only 2 minutes • 100% confidential')}
      </p>
    </main>
  );
}
